...
from fetch_products import fetch_product, fetch_products
...
def handle_menu(update: Update, context: CallbackContext) -> str:
    ...
    reply_markup = InlineKeyboardMarkup(
        [[InlineKeyboardButton("Назад к списку", callback_data=BACK_TO_PRODUCTS_CALLBACK)]]
    )
    update.effective_message.reply_text(
...
def echo(update: Update, context: CallbackContext) -> str:
    ...
    update.effective_message.reply_text(user_reply)
    return ECHO_STATE
...
def main() -> None:
    ...
    if not token:
        raise RuntimeError("TG_TOKEN is missing in .env")
    ...
    logging.info("Echo state-machine bot started")
    updater.idle()
